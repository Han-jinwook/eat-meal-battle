...
exports.handler = async (event, context) => {
  ...
  try {
    ...
    try {
      ...
      console.log('이미지 접근 불가로 검증 실패 처리');
      
      // 검증 실패 처리 - 이미지 URL에 접근할 수 없으면 검증 불가
      const status = 'rejected';
      const { error: updateError } = await supabaseAdmin
        .from('meal_images')
        .update({
          status: status,
          match_score: 0, // 0% - 검증 불가
          explanation: '이미지 URL에 접근할 수 없어 검증이 불가능합니다. 이미지를 다시 업로드해주세요.'
        })
        .eq('id', imageId);
        
      if (updateError) {
        console.error('검증 실패 저장 오류:', updateError);
        return {
          statusCode: 500,
          headers,
...
한국어로 응답해주세요.`
      ...
      // OpenAI API 오류 시 검증 실패 처리
      console.log('OpenAI API 오류로 검증 실패 처리');
      
      // 검증 실패 처리 - AI 검증 서비스 오류로 검증 불가
      const status = 'rejected';
      const { error: updateError } = await supabaseAdmin
        .from('meal_images')
        .update({
          status: status,
          match_score: 0, // 0% - 검증 불가
          explanation: 'AI 검증 서비스 오류로 검증이 불가능합니다. 잠시 후 다시 시도해주세요.'
        })
        .eq('id', imageId);
