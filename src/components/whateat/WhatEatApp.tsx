    ...
    <div className="min-h-screen bg-gradient-to-br from-[#fffaf5] via-[#fff7ed] to-[#fffbf2]">
      ...
      <div className="relative flex justify-center min-h-screen pt-[62px]">
        ...
        <div className="w-full max-w-[800px] min-h-screen flex flex-col relative shadow-2xl shadow-black/5 bg-gradient-to-br from-[#fffaf5] via-[#fff7ed] to-[#fffbf2] shrink-0 z-10">
          ...
          <div className={cn("sticky top-[62px] z-40 bg-gradient-to-b from-[#fffaf5] via-[#fff7ed] to-[#fffbf2] px-5 lg:px-8 pt-2", bottomNavTab === "home" ? "pb-0" : "pb-1 border-b border-muted/10")}>
            {bottomNavTab === "solo" && (
              <TabNavigation
                activeTab={activeTab}
        ...
        {/* 
          우측 사이드 광고 배너 영역 
          - flex 레이아웃 흐름에 간섭하지 않고 absolute 포지셔닝으로 800px 컨테이너 바깥 우측에 고정 배치합니다.
          - 이를 통해 브라우저 크기에 상관없이 본문 800px 컨테이너가 항상 독립적인 탭 중앙에 위치하도록 보장합니다.
        */}
        <aside className="hidden lg:flex w-[160px] shrink-0 items-start justify-center pt-20 absolute top-[62px] left-[calc(50%+400px+8px)] h-[calc(100vh-62px)]">
          <div className="w-[140px] h-[400px] bg-white/50 border border-dashed border-muted/30 rounded-2xl flex items-center justify-center text-muted-foreground/50 text-xs sticky top-[82px]">
            AD BANNER
          </div>
        </aside>
      </div>
