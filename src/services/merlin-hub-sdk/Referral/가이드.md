...
// 광고 배너와 함께 세로 스택 구성 시, 개별 카드가 아닌 공통 Wrapper에 sticky를 일임하여 겹침을 방지합니다.
<aside className="hidden lg:flex w-[160px] shrink-0 absolute top-[62px] left-[calc(50%+400px+8px)] h-[calc(100vh-62px)]">
...
### 4단계: 피초대자 탈퇴 시 실적 자동 정산
- 피초대자가 탈퇴(`deleted_at`)하는 즉시, 초대자의 `HubHistoryList` (앱 초대 List) 목록에서 실시간으로 자동 제외되어 오직 실제 활동 중인 유효 친구만 표시됩니다.
