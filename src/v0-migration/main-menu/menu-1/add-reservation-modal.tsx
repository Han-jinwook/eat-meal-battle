...
export function AddReservationModal({ isOpen, onClose, initialUrl, editData }: AddReservationModalProps) {
...
const handleSubmit = () => {
    console.log({ menuName, date, mealTime, memo, mealType, selectedPlace, deliveryStoreName })
    onClose()
  }
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6">
      ...
      {/* Modal */}
      <div className="relative w-full max-w-md bg-background rounded-3xl shadow-[0_30px_100px_rgba(0,0,0,0.15)] border border-white overflow-hidden max-h-[calc(100vh-120px)] overflow-y-auto hide-scrollbar mb-20">
        {/* Close Button */}
        <button 
          onClick={onClose}
              ...
              {urlPreview.isLoading ? (
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div className="flex-1 space-y-2">
                      <div className="flex items-center gap-2 text-primary">
                        ...
                        <span className="text-sm font-medium">URL 분석 중...</span>
                      </div>
                ...
                <div className="bg-white rounded-2xl overflow-hidden border border-gray-100">
                  ...
                  <div className="relative h-40 bg-muted">
                    ...
                    />
                    <div className="absolute top-3 left-3 px-2 py-1 bg-black/60 rounded-lg flex items-center gap-1.5">
                      <Link2 className="size-3 text-white" />
                      <span className="text-[10px] text-white font-medium truncate max-w-[150px]">
                        {urlPreview.url}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="p-4">
                    ...
                    <input
                      ...
                      className="w-full px-3 py-2.5 bg-gray-50 border-2 border-transparent rounded-lg text-sm font-bold text-foreground focus:border-primary focus:bg-white outline-none transition-all"
          ...
          <div className="space-y-5">
            ...
            <div className="flex flex-col gap-3">
                    ...
                    <button
                      ...
                      className={cn(
                        ...
                        mealType === type.id
                          ...
                          : "bg-white border-2 border-gray-100 text-foreground hover:border-primary/30"
                      )}
                    >
              ...
              {mealType === "집밥" && (
                <div className="mt-2">
                  <input
                    className="w-full px-4 py-3.5 bg-white border-2 border-gray-100 rounded-xl focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none transition-all text-foreground text-sm placeholder:text-muted-foreground/50"
                    placeholder="레시피 URL 입력 (예: https://youtube.com/...)"
                    type="url"
                    value={recipeUrl}
              ...
              {mealType === "외식" && (
                <div className="mt-2">
                  <input
                    className="w-full px-4 py-3.5 bg-white border-2 border-gray-100 rounded-xl focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none transition-all text-foreground text-sm placeholder:text-muted-foreground/50"
                    placeholder="장소 URL 입력 (예: https://naver.me/...)"
                    type="url"
                    value={recipeUrl}
                    onChange={(e) => setRecipeUrl(e.target.value)}
                    onKeyDown={(e) => {
              ...
              {mealType === "배달" && (
                <div className="mt-2">
                  <input
                    className="w-full px-4 py-3.5 bg-white border-2 border-gray-100 rounded-xl focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none transition-all text-foreground text-sm placeholder:text-muted-foreground/50"
                    placeholder="식당명 입력 (예: BHC치킨 강남점)"
                    type="text"
                    value={deliveryStoreName}
                    onChange={(e) => setDeliveryStoreName(e.target.value)}
                    onKeyDown={(e) => {
                      if (e.key === "Enter") {
                        e.preventDefault()
                        handleSubmit()
            ...
            <div className="flex flex-col gap-3">
              ...
              <input
                type="text"
                className="w-full px-4 py-3 bg-white border-2 border-gray-100 rounded-xl focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none transition-all text-foreground text-sm placeholder:text-muted-foreground/50"
            ...
            <div className="flex flex-col gap-3">
              ...
              {/* 선택된 날짜 표시 */}
              {date && (
                <div className="px-4 py-2.5 bg-orange-50 rounded-xl">
                  <p className="text-sm font-medium text-primary">{formatDateDisplay(date)}</p>
                </div>
              )}
              {/* 직접 선택 date picker */}
              {showDatePicker && (
                <input
                  className="w-full px-4 py-3.5 bg-white border-2 border-gray-100 rounded-xl focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none transition-all text-foreground text-sm"
                  type="date"
                  value={date}
            ...
            {mealType === "외식" && (
              <div className="flex flex-col gap-3">
                ...
                {selectedPlace ? (
                  <div className="relative p-3 bg-white border-2 border-primary/30 rounded-xl">
                  ...
                  <button
                    ...
                    className="w-full px-4 py-3.5 bg-white border-2 border-dashed border-gray-200 hover:border-primary/50 rounded-xl transition-all flex items-center gap-3 group"
                  >
...
{/* Submit Buttons */}
  ...
  className="flex-1 flex items-center justify-center gap-2 px-6 py-4 bg-primary text-white rounded-xl shadow-lg hover:bg-orange-600 hover:scale-[1.02] active:scale-95 transition-all font-bold text-sm"
  >
  ...
  {isEditMode ? "수정 완료" : "예약 저장하기"}
        ...
        {/* Place Search Modal */}
        {showPlaceSearch && (
          <div className="absolute inset-0 z-10 flex flex-col bg-background rounded-3xl overflow-hidden">
            {/* Header */}
            <div className="p-6 border-b border-muted/50">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-11 pr-4 py-3 bg-muted/50 rounded-xl text-foreground text-sm outline-none focus:ring-2 focus:ring-primary/20"
                  autoFocus
                />
              </div>
            </div>
